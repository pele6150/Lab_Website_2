...
function loadStatsPage(){
	var statsTab = document.getElementById("stats_table");
	var cuWinCount = 0;
	var cuLossCount = 0;
	for(i = 2; i < statsTab.rows.length; i++){
		if(statsTab.rows.item(i).cells.item(2).innerHTML > statsTab.rows.item(i).cells.item(3).innerHTML){
			statsTab.rows.item(i).cells.item(4).innerHTML = "CU Boulder";
			cuWinCount++;
		}
		else{
			statsTab.rows.item(i).cells.item(4).innerHTML = statsTab.rows.item(i).cells.item(1).innerHTML;
			cuLossCount++;
		}
	}
	document.getElementById("wins").innerHTML = cuWinCount;
	document.getElementById("losses").innerHTML = cuLossCount;
}
...
function loadPlayersPage(){
	...
	for(i = 0; i < players.length; i++){
		currPlayer = document.createElement('a');
		currPlayer.href = "#";
		currPlayer.index = i;
		currPlayer.onclick = function() {switchPlayers(this.index)};
		currPlayer.text = players[i].name;
		currPlayer.className = "dropdown-item";
		document.getElementById("player_selector").appendChild(currPlayer);
	}
}
